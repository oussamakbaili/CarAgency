...
class MessageController extends Controller
{
    ...
    public function index()
    {
        ...
        // Récupérer les réservations actives avec messages pour cette agence
        $rentals = Rental::where('agency_id', $agency->id)
            ->where('status', 'active')
            ->with(['car', 'user.client', 'messages' => function($query) use ($user) {
        ...
        foreach ($rentals as $rental) {
            $rental->unread_count = $rental->getUnreadMessagesCountForUser($user->id, 'agency');
            $rental->last_message = $rental->messages->first();
            $rental->conversation_type = 'rental';
        }

        // Récupérer UNIQUEMENT les tickets de support créés par cette agence
        // (où agency_id correspond à l'agence connectée et client_id est null)
        $supportTickets = SupportTicket::where('agency_id', $agency->id)
            ->whereNull('client_id') // S'assurer que c'est un ticket créé par l'agence, pas par un client
            ->with(['messages' => function($query) use ($agency) {
                $query->orderBy('created_at', 'desc');
            }])
        ...
        foreach ($supportTickets as $ticket) {
            $ticket->unread_count = $ticket->getUnreadMessagesCount($agency);
        ...
        $allConversations = collect()
            ->merge($rentals->map(function($rental) {
                return (object) [
                    ...
                    'subtitle' => 'Client: ' . $rental->user->name . ' • ' . $rental->start_date->format('d/m/Y') . ' - ' . $rental->end_date->format('d/m/Y'),
        ...
        return view('agence.messages.unified', compact('allConversations', 'rentals', 'supportTickets'));
    }
