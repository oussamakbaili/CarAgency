                    ...
                    <div class="flex items-center space-x-2">
                        ...
                        <div class="relative">
                            <select id="conversation-filter" onchange="filterConversations(this.value)" class="appearance-none bg-white border border-gray-300 rounded-lg px-3 py-1.5 pr-8 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent">
                                ...
                                <option value="support" {{ request('filter') === 'support' ? 'selected' : '' }}>Tickets Support</option>
                                <option value="rental" {{ request('filter') === 'rental' ? 'selected' : '' }}>Messages Clients</option>
                            </select>
                                        ...
                                        <div class="flex items-center justify-between">
                                            <p class="text-sm text-gray-600 truncate flex-1">
                                                @if($conversation->type === 'rental')
                                                    ...
                                                    @endif
                                                @else
                                                    @if($conversation->last_message->sender_type === 'App\Models\Agency')
                                                        <span class="font-medium text-orange-600">Vous:</span>
                                                    @else
        ...
        <div class="flex-1 bg-white flex flex-col">
            ...
            <div id="welcome-screen" class="flex-1 flex items-center justify-center bg-gray-50">
                <div class="text-center">
                    ...
                    <h2 class="text-2xl font-semibold text-gray-900 mb-2">TOUBCAR Messages Agence</h2>
                    <p class="text-gray-600 max-w-md mx-auto">
                        Sélectionnez une conversation pour communiquer avec vos clients ou contacter le support.
                    </p>
                </div>
            </div>
...
function loadMessages(conversation) {
    ...
    setTimeout(() => {
        if (conversation.last_message) {
            // Determine if the message is from the agency or client
            const isFromAgency = conversation.last_message.sender_type === 'agency';
            ...
            const senderLabel = isFromAgency ? 'Vous' : 'Client';
...
async function loadSupportMessages(conversation) {
    ...
    try {
        ...
        const response = await fetch(`/agence/support/messages/${ticketId}`, {
            headers: {
...
function displaySupportMessages(messages) {
    ...
    messages.forEach(message => {
        ...
        const senderLabel = isFromAgency ? 'Vous' : 'Support';
