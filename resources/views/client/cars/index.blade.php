...
@section('content')
<!-- Page Header -->
<div class="mb-4 sm:mb-6 reveal-section">
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
        <div>
            <h1 class="text-xl sm:text-2xl font-semibold text-gray-900">Parcourir les Véhicules</h1>
            <p class="text-sm sm:text-base text-gray-600 mt-1">Trouvez le véhicule parfait pour votre prochaine location</p>
        </div>
...
<div class="bg-white overflow-hidden shadow-sm rounded-lg mb-4 sm:mb-6 reveal-section">
    <div class="p-4 sm:p-6">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 sm:mb-6 gap-3 sm:gap-4">
            <h3 class="text-base sm:text-lg md:text-xl font-semibold text-gray-900">Filtres de Recherche</h3>
        ...
        <form method="GET" action="{{ route('client.cars.index') }}" class="space-y-4 sm:space-y-6" id="carsFiltersForm">
            <!-- Main Search -->
            <div class="relative">
                ...
                <input type="text" name="search" value="{{ request('search') }}" 
                       ...
                       class="block w-full pl-9 sm:pl-10 pr-3 py-2.5 sm:py-3 text-sm sm:text-base border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500">
            </div>

            <!-- Filters Grid -->
            <div id="filtersGrid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                <!-- Brand Filter -->
                <div>
                ...
                <!-- Max Price -->
                <div>
                    <label for="max_price" class="block text-xs sm:text-sm font-medium text-gray-700 mb-1 sm:mb-2">Prix max/jour</label>
                    <div class="relative">
                        <input type="number" id="max_price" name="max_price" value="{{ request('max_price') }}" 
                               placeholder="100"
                               class="w-full px-3 sm:px-4 py-2 pr-8 text-xs sm:text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-orange-500">
                        <span class="absolute right-3 top-2 text-xs sm:text-sm text-gray-500">MAD</span>
                    </div>
                </div>

                <!-- Year From -->
                <div>
                    <label for="year_from" class="block text-xs sm:text-sm font-medium text-gray-700 mb-1 sm:mb-2">Année min</label>
    ...
    <div class="bg-white overflow-hidden shadow-sm rounded-lg hover:shadow-md transition-shadow border border-gray-200 group">
        ...
        <div class="relative aspect-video bg-gray-200 overflow-hidden">
            ...
            <div class="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-30 transition-all duration-300 flex items-center justify-center opacity-0 group-hover:opacity-100">
                <div class="flex flex-col sm:flex-row gap-2 px-2">
                    ...
                    @else
                        <span class="px-3 sm:px-4 py-1.5 sm:py-2 bg-gray-400 text-white rounded-lg cursor-not-allowed text-xs sm:text-sm font-medium">
                            Indisponible
                        </span>
                    @endif
                </div>
        ...
        <div class="p-4 sm:p-6">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-start mb-2 sm:mb-3 gap-2">
                <div class="flex-1">
                    <h3 class="text-base sm:text-lg font-semibold text-gray-900 group-hover:text-orange-600 transition-colors">
                        {{ $car->brand }} {{ $car->model }}
                    </h3>
                    <p class="text-xs sm:text-sm text-gray-600 mt-0.5">{{ $car->year }} • {{ ucfirst($car->fuel_type) }}</p>
                </div>
            ...
            <div class="space-y-1.5 sm:space-y-2 mb-3 sm:mb-4">
                ...
                <div class="flex items-center text-xs sm:text-sm text-gray-600">
                    ...
                    <span class="truncate">{{ $car->registration_number }}</span>
                </div>
            </div>
            ...
            <div class="flex flex-col sm:flex-row gap-2">
                ...
                @else
                    <button disabled
                            class="flex-1 px-3 sm:px-4 py-2 bg-gray-400 text-white rounded-lg cursor-not-allowed text-center text-xs sm:text-sm font-medium">
                        Indisponible
                    </button>
                @endif
            </div>
...
<div class="bg-white overflow-hidden shadow-sm rounded-lg reveal-section">
    <div class="p-8 sm:p-12 text-center">
        ...
        <h3 class="text-base sm:text-lg font-medium text-gray-900 mb-1 sm:mb-2">Aucun véhicule trouvé</h3>
        <p class="text-sm sm:text-base text-gray-600 mb-4 sm:mb-6">Essayez de modifier vos critères de recherche pour trouver plus de véhicules.</p>
        <div class="flex flex-col sm:flex-row justify-center gap-2 sm:gap-4">
            <a href="{{ route('client.cars.index') }}" 
               class="inline-flex items-center justify-center px-4 sm:px-6 py-2 sm:py-2.5 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors text-xs sm:text-sm font-medium">
                Voir tous les véhicules
            </a>
            <button onclick="clearFilters()" 
                    class="inline-flex items-center justify-center px-4 sm:px-6 py-2 sm:py-2.5 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition-colors text-xs sm:text-sm font-medium">
                Effacer les filtres
            </button>
        </div>
    </div>
